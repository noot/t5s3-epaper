...
#[esp_hal::ram(unstable(rtc_fast, persistent))]
static mut LAST_SYNC_UNIX: u64 = 0;
...
const INFO_REFRESH_TICKS: u16 = 40;
...
fn draw_status_bar(display: &mut Display, pct: u16, time: Option<(u32, u32)>) {
...
fn read_info(display: &mut Display, clock: &mut Clock) -> (f32, i8, u64, Option<u64>) {
    ...
    let last_sync = unsafe { LAST_SYNC_UNIX };
...
fn info_values_rect() -> lilygo_t5s3paperpro::display::Rectangle {
...
fn kb_hit(sx: i32, sy: i32, symbols: bool, shift: bool) -> Option<Key> {
...
fn draw_keyboard(display: &mut Display, symbols: bool, shift: bool) {
...
fn draw_message(display: &mut Display, message: &str) {
...
fn draw_lora_status(display: &mut Display, status: &str) {
